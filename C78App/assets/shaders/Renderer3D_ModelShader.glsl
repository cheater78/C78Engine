...
void main() {
	vec4 texColor = textureColor(int(v_TexIndex), v_Color, v_TexCoord);
	vec4 ambient = u_AmbientLight.color;
	...
	o_Color = vec4(( ambient.xyz * ambient.w + (diffuseLight * diffuse + specLight * specular)) * texColor.xyz, dissolve * texColor.w );
}
