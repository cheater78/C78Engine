...
namespace C78Editor {
    ...
    void EnttInspector::showEntityList(C78E::Ref<C78E::Scene> scene) {
        ...
        ImGui::BeginTable("Entities", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_NoClip | ImGuiTableFlags_SizingStretchProp | ImGuiTableFlags_PreciseWidths);
        ImGui::TableNextRow();
        {
        ...
        for (auto entt : entts) {
            ...
            ImGui::TableNextRow();
            {
                ImGui::TableNextColumn();
                // Loaded
                if (ImGui::Checkbox(std::to_string((uint32_t)entt).c_str(), &entity.getComponent<C78E::StateComponent>().enable));// { entity.getComponent<C78E::StateComponent>().enable = !entity.getComponent<C78E::StateComponent>().enable; }
                //--------------------------------------------
                ImGui::TableNextColumn();
                // Name
