...
namespace C78E {
	...
	void Log::init(){
		const char* formatterPattern = "%^[%T] %n: %v%$";

		s_RingbufferSink = std::make_shared<spdlog::sinks::ringbuffer_sink_mt>(c_MaxLogLines);
		s_RingbufferSink->set_pattern(formatterPattern);
		s_FileLoggerSink = std::make_shared<spdlog::sinks::basic_file_sink_mt>("C78Engine.log");
		s_FileLoggerSink->set_pattern(formatterPattern);
...

		{
			...
			coreSink->set_pattern(formatterPattern);
...

		{
			...
			appSink->set_pattern(formatterPattern);
