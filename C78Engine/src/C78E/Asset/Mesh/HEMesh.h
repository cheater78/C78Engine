...
namespace C78E {
	...
	class HEMesh : public Asset {
	public:
		static Ref<HEMesh> createMesh();
	...
	public:
		...
		class MeshIterator {
		protected:
			MeshIterator(HEMesh& mesh, const std::vector<Index>& elements, std::function<T(HEMesh&, Index)> at) : m_Mesh{ mesh }, m_Indecies{ elements }, m_At{ at } { reset(); }
		...
		public:
			...
			T get() { C78E_CORE_ASSERT(m_Current != -1, "HEMesh::MeshIterator: operator*() cant be called on end!"); return m_At(m_Mesh, m_Indecies[m_Current]); }
			const T get() const { C78E_CORE_ASSERT(m_Current != -1, "HEMesh::MeshIterator: operator*() cant be called on end!"); return m_At(m_Mesh, m_Indecies.at(m_Current)); }
		...
		protected:
			...
			Index m_Current = -1;
			std::function<T(HEMesh&, Index)> m_At;
		};

		class VertexIterator : public MeshIterator<VertexHandle> {
		protected:
			static VertexHandle access(HEMesh& mesh, VertexIndex index) { return VertexHandle(mesh, index); }
		public:
			VertexIterator(HEMesh& mesh, const std::vector<Index>& elements) : MeshIterator<VertexHandle>(mesh, elements, access) {}
		};

		class HalfedgeIterator : public MeshIterator<HalfedgeHandle> {
		protected:
			static HalfedgeHandle access(HEMesh& mesh, HalfedgeIndex index) { return HalfedgeHandle(mesh, index); }
		public:
			HalfedgeIterator(HEMesh& mesh, const std::vector<Index>& elements) : MeshIterator<HalfedgeHandle>(mesh, elements, access) {}
		};

		class FaceIterator : public MeshIterator<FaceHandle> {
		protected:
			static FaceHandle access(HEMesh& mesh, FaceIndex index) { return FaceHandle(mesh, index); }
		public:
			FaceIterator(HEMesh& mesh, const std::vector<Index>& elements) : MeshIterator<FaceHandle>(mesh, elements, access) {}
		};

		class PositionIterator : public MeshIterator<Position> {
		protected:
			static Position access(HEMesh& mesh, VertexIndex index) { return mesh.m_Positions[index]; }
		public:
			PositionIterator(HEMesh& mesh, const std::vector<Index>& elements) : MeshIterator<Position>(mesh, elements, access) {}
		};

		class NormalIterator : public MeshIterator<Normal> {
		protected:
			static Normal access(HEMesh& mesh, NormalIndex index) { return mesh.m_Normals[index]; }
		public:
			NormalIterator(HEMesh& mesh, const std::vector<Index>& elements) : MeshIterator<Normal>(mesh, elements, access) {}
		};

		class ColorIterator : public MeshIterator<Color> {
		protected:
			static Color access(HEMesh& mesh, ColorIndex index) { return mesh.m_Colors[index]; }
		public:
			ColorIterator(HEMesh& mesh, const std::vector<Index>& elements) : MeshIterator<Color>(mesh, elements, access) {}
		};

		class TextureCoordinateIterator : public MeshIterator<TextureCoordinate> {
		protected:
			static TextureCoordinate access(HEMesh& mesh, TextureCoordinateIndex index) { return mesh.m_TextureCoordinates[index]; }
		public:
			TextureCoordinateIterator(HEMesh& mesh, const std::vector<Index>& elements) : MeshIterator<TextureCoordinate>(mesh, elements, access) {}
		};


	private: // HEMesh Topology
		struct BaseHandle {
		protected:
			BaseHandle() = delete;
			BaseHandle(HEMesh& mesh, Index index) : m_Mesh(mesh), m_Index(index) { }
			~BaseHandle() = default;
		public:
			virtual HEMesh& mesh() final { return m_Mesh; }
	...
	public:
		...
		public:
			VertexHandle() = delete;
			VertexHandle(HEMesh& mesh, VertexIndex index) : BaseHandle(mesh, index) { }
		...
		public:
			...
			HalfedgeHandle(HEMesh& mesh, HalfedgeIndex index) : BaseHandle(mesh, index) { }
		...
		public:
			FaceHandle() = delete;
			FaceHandle(HEMesh& mesh, FaceIndex index) : BaseHandle(mesh, index) { }
			~FaceHandle() = default;
	...
	private:
		...
		protected:
			...
			BasePropertyHandle(HEMesh& mesh, PropertyIndex propertyIndex) : m_Mesh(mesh), m_PropertyIndex(propertyIndex) { }
	...
	public:
		...
		protected:
			...
			VPropertyHandle(HEMesh& mesh, PropertyIndex propertyIndex) : BasePropertyHandle<T>(mesh, propertyIndex) { }
		...
		protected:
			...
			HPropertyHandle(HEMesh& mesh, PropertyIndex propertyIndex) : BasePropertyHandle<T>(mesh, propertyIndex) { }
		...
		protected:
			...
			FPropertyHandle(HEMesh& mesh, PropertyIndex propertyIndex) : BasePropertyHandle<T>(mesh, propertyIndex) { }
	...
	public:
		...
		VertexHandle halfedgeFrom(HalfedgeIndex halfedgeIndex);
		VertexHandle halfedgeTo(HalfedgeIndex halfedgeIndex);
		HalfedgeIterator halfedges();
